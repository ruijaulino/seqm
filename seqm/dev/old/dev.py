...
class Array:
    ...
    def set_values(self, values:np.ndarray, copy:bool = True):
        self.values = np.copy(values) if copy else values
        self._checks()
...
class Data:
    ...
    def _checks(self):
        ...
        # make sure w and s exits otherwise create them
        # it makes sense that data objects carry this info
        if 's' not in self.arrays:
            self.create_s()
    ...
    def create_s(self):
        self.s = Array(np.zeros(self.n), STRATEGY_COL)

    def create_w(self):
        c = [f"{WEIGHT_PREFIX}_{e}" for e in self.y.cols]
        self.w = Array(np.zeros((self.n, self.p)), c)
    ...
    def __setattr__(self, name, value):
        ...
        if isinstance(value, Array):
            self.arrays.update({name: copy.deepcopy(value)})
            self._checks()
        elif isinstance(value, np.ndarray):
            # check if arrays has the key and set the array
            # create s or w in case they do not exits / eliminate complexity from other parts
            if name in self.arrays:                
                self.arrays[name].set_values(value)
                self._checks()
            else:
                raise Exception(f"Array {name} need to be created first")
        else:
            # regular working
    ...
    def model_input(self, idx = None):
        ...
        if idx is None: idx = self.n
        start = self.msidx.start[idx] 
        # TO DO: try to avoid excessive number of copies here
        print(start, idx, self.n-1)
        print(self[start:idx].as_dict())
        print(Sdfsdf)
