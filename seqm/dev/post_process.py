...
def portfolio_post_process(paths:List[Dict[str,pd.DataFrame]],pct_fee=0.,seq_fees=False,sr_mult=1,n_boot=1000,view_weights=True,use_pw=True,multiplier=1,start_date='',end_date='', n_boot_datasets:int = None, output_paths:bool = True, sr_pw = False, *args, **kwargs):
	...
	for path in paths:
		for k in range(n_boot_datasets):
			...
			for key in keys_sample:
				...
				if sr_pw:
					tmp = np.mean(s)/np.std(s)
					pw=pd.DataFrame(np.ones_like(pw.values)*tmp.values.ravel()[0],columns=pw.columns,index=pw.index)	
				else:
					if not use_pw:
