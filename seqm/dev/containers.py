...
class Data:
	def __init__(self, y, x, z, idx, t, oos_s, ts, x_cols, t_cols, y_cols, safe_arrays = True):
		# add unix timestamp
		...
		self.oos_s = np.zeros(self.y.shape[0]) if oos_s is None else np.copy(oos_s) if safe_arrays else oos_s
	...
	def from_df(cls, df: pd.DataFrame, safe_arrays = True):
		...
		oos_s = np.zeros(y.shape[0])
		return cls(y, x, z, idx, t, oos_s, ts, x_cols, t_cols, y_cols, safe_arrays)
	...
	def _from_idx(self, idx:np.ndarray, create_new = False):		
		...
		oos_s_ = self.oos_s[idx] 
		if create_new:
			return Data(y_, x_, z_, idx_, t_, oos_s_, ts_, self.x_cols, self.t_cols, self.y_cols, safe_arrays=True)
		else:
	...
	def set_oos_s(self, s, ts_lower, ts_upper, create_new = True):
		if self.empty: return self
		idx = np.where( (self.ts<=ts_upper) & (self.ts>=ts_lower) )[0]
		self.oos_s[idx] = s
	...
	def build_train_inputs(self):
		...
		'''
		return {'x' : self.x, 'y' : self.y, 'z' : self.z, 'oos_s':self.oos_s, 'idx':self.converted_idx(), 't': self.t} 
...
class Dataset:
	...
	# specific methods
	def set_oos_s(self, model_pipe, fold_index):
		for key, data in self.items():
			ts_lower, ts_upper = self.folds_ts[fold_index]			
			if key in model_pipe.keys():
				data.set_oos_s(model_pipe[key].s, ts_lower, ts_upper)
