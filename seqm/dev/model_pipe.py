...
class BaseModelPipe:
	...
	def set_oos_s(self, oos_s):
		'''
		oos_s: numpy (m, ) array with a sequence of strategy returns
		'''
		self.oos_s = oos_s
...
class ModelPipe:
	...
	def set_datasets(self, train_dataset, test_dataset):
		'''
		assign datasets to base model pipes
		'''
		pass
	...
	def estimate(self):
		...
		if self.master_model is not None:
			...
			for k, e in self.items(): 
				# need to call estimate_pw because the model is estimated globally
