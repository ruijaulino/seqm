...
def approx_nested_loocv_linear_model_select_new(X: np.ndarray, y: np.ndarray):
    '''
    Approximates out-of-sample Sharpe ratio while penalizing based on sample size,
    including during the nested LOOCV approximation.
    ...
    best_feature = np.argmax(sharpe_penalized)
    ...
    oos_sharpe = sharpe_penalized[best_feature]
...
def intraday_linear_model_select_search_base(
    ...
    try:
        ...
        # check of there are enough points in y to proceed with calculations
        if np.sum(np.isnan(y)) / n >= nan_fraction_th:
            return -1, -1, -1, -1 
        X, features_desc = build_features(px_matrix, target_start)
        # check data for valid points
        nan_fraction = np.sum(np.isnan(X), axis = 0)/X.shape[0]
        valid_features = nan_fraction < nan_fraction_th
        X = X[:,valid_features]
        if X.size == 0:
            return -1, -1, -1, -1
        # fill na in X
        ...
        features_desc = features_desc[valid_features]
        feature, nested_sr, sr = approx_nested_loocv_linear_model_select(X, y)
...
def intraday_linear_model_select_search(
    ...
    nan_frac_px = np.sum(np.isnan(px_matrix), axis = 0)/px_matrix.shape[0]
    
    nan_fraction_th = np.quantile(nan_frac_px, nan_fraction_quantile)
    print('NaN threshold: ', nan_fraction_th)
